...
class QueueManager(private val pref: SharedPreferencesRetriever) {
    ...
    fun push(url: String, element: String) {
        ...
        pref.getDefaultSharedEditor().putStringSet(SHARED_QUEUE_MANAGER_QUEUE_URL, HashSet<String>(queueUrl)).apply()
        pref.getDefaultSharedEditor().putStringSet(SHARED_QUEUE_MANAGER_QUEUE, HashSet<String>(queue)).apply()
